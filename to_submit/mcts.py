...
class MCTS():
    ...
    def search(self, state):
        ...
        while time.time() < timeLimit:
            self.dive()
        bestIndex = self.getBestChild(self.root, 0) # set explorationvalue as 0 as want to exploit the best move
    ...
    def expand(self, node):
        """
        expand creates an mcts node of state as well as expands it
        """
        node.get_children()
        node.to_mcts()
    ...
    def getBestChild(self, node, explorationValue):
        """
        returns the index of the best child depending on the exploration value given
        """
