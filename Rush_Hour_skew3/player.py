...
class Search_Node:
    ...
    def is_terminal(self):
        if self.parent and self.features.score[self.turn][1] > self.parent.features.score[self.turn][1]:
            return True
        return False
    ...
    def total_less_than_three(self):
        return 0*self.manhat() + 12*self.eat() + 0*self.leave()

    def total_three(self):
        return self.manhat() + 6*self.eat() + 12*self.leave()

    def total_four(self):
        return self.manhat() + 6*self.eat() + 12*self.leave()

    def total_five(self):
        return 2*self.manhat() + 6*self.eat() + 12*self.leave()

    def total_more_than_five(self):
        return 2*self.manhat() + 0*self.eat() + 12*self.leave()
    ...
    def choose_function(self):
        total = sum(self.features.score[self.turn])
        if total <= 2:
            return self.total_less_than_three()
        elif total == 3:
            return self.total_three()
        elif total == 4:
            return self.total_four()
        elif total == 5:
            return self.total_five()
        else:
...
class Minimax:
    ...
    def _minimax(self, state, depth):
        ...
        for child in state.children:
            ...
            if next_state == None:
                continue
            value = next_state.choose_function()
            if (value > best_value):
...
class MCTS():
    ...
    def dive(self):
        ...
        node2 = node
        if node2.is_terminal() is True:
            reward = node2.getReward(self.root, delay = 1)
        else:
            feature, delay = node2.simulation()
